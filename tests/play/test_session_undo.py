...
def _new_session(dice_mode=DiceMode.AUTO, human_color=Color.WHITE, starting_player=Color.WHITE):
    return PlaySession(
        config=_config(),
        agent=None,
...
class TestSessionInitial(unittest.TestCase):
    def test_initial_history_has_one_snapshot(self):
        ...
        self.assertEqual(s.history[0].next_player, Color.WHITE)
...
class TestUndoTerminal(unittest.TestCase):
    ...
    def _setup_white_one_move_from_winning(self, s):
        """14 white in bear-off slot 25, 1 white at point 23. White rolls (2,5):
        the move 23 -> 25 bears off the last piece and wins."""
        board_size = s.game.board.board_size
        for i in range(0, board_size + 2):
            s.game.board.points[i] = Point(i)
        s.game.board.points[board_size + 1] = Point(board_size + 1, Color.WHITE, 14)
    ...
    def test_undo_unflips_terminal(self):
        ...
        self.assertEqual(s.winner(), Color.WHITE)
        s.undo()
...
class TestUndoPin(unittest.TestCase):
    ...
    def test_undo_releases_pinned_checker(self):
        ...
        for i in range(0, board_size + 2):
            s.game.board.points[i] = Point(i)
        s.game.board.points[18] = Point(18, Color.WHITE, 1)
        s.game.board.points[21] = Point(21, Color.BLACK, 1)
        # Park more pieces so the game isn't trivially over either side.
        s.game.board.points[1] = Point(1, Color.WHITE, 14)
        s.game.board.points[24] = Point(24, Color.BLACK, 14)

        pin_move = Move([HalfMove(s.game.board.points[18], s.game.board.points[21], Color.WHITE)])
        s.set_dice(3, 4)
        s.commit_move(pin_move)

        # Pin applied: point 21 has [BLACK, WHITE]; point 18 empty.
        self.assertEqual(len(s.game.board.points[18].pieces), 0)
        self.assertEqual(s.game.board.points[21].pieces, [Color.BLACK, Color.WHITE])

        s.undo()
        # Pin released; original layout restored.
        self.assertEqual(s.game.board.points[18].pieces, [Color.WHITE])
        self.assertEqual(s.game.board.points[21].pieces, [Color.BLACK])
