...
class GameBoard:
    ...
    def initialize_board(self):
        self.points[24] = Point("W", 15)
        self.points[1] = Point("B", 15)

    def move_checker(self, from_point, to_point):
        if self.is_point_open(to_point) or (len(self.points[to_point]) == 1 and self.points[to_point][0] != self.points[from_point][0]):
            checker = self.points[from_point].pop()
            self.points[to_point].append(checker)

    def is_point_open(self, point):
        return len(self.points[point]) == 0 or (len(self.points[point]) == 1 and self.points[point][0] == self.points[point][0])
