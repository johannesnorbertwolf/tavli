...
`evaluate_lookahead_selfplay(config, model_path, games_per_color, num_workers)` builds the full task list of `(flex_color, seed)` games, round-robins them into per-worker chunks, and runs them with a `multiprocessing` *spawn* `Pool`. Unlike training there is no weight queue (the model is static). Each worker sets `torch.set_num_threads(1)` to avoid thread contention, plays its chunk via `_play_one_game`, and returns `(flex_wins, n_games, depth_histogram, move_times)`. The parent aggregates and prints: the flexible win rate with a Wilson 95% CI and a two-sided binomial p-value vs 0.5 (`_wilson_interval`, `_two_sided_binomial_p`); a histogram of `agent.last_search_depth` over all flexible moves (the "how far did it actually look" answer); and avg/median/min/max flexible move time.

Because flexible moves are budget-bound (~`play_time_budget_s` each), a full `games_per_color=100` run is multi-hour even on 6 workers — start it yourself rather than expecting it to finish inline.
