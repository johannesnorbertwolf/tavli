...
| Type | What it is |
|---|---|
| `GameSession` (`@MainActor`, `ObservableObject`) | Owns `Game`, drives the turn state machine (`awaitingRoll/picking/moving/aiThinking/animating/gameOver`); publishes the view contract; optional Core ML AI side (multi-ply expectimax search off the main actor); two-surface undo; `onGameOver` hook; **attempt mode** (`onMoveAttempt`) + `drill(...)` seeder for the post-game drill (#63). |
...
| `GameReview.swift` (#62, #63) | Post-game blunder analysis: replays a `GameRecord`, re-ranks every human ply at 3-ply, returns the moves that fell ≥10% short of the AI's best. On-device port of the CLI `review` command. Also exposes `Agent.scoreCandidate` (single-move grading for the drill). See `REFERENCE.md`. |
