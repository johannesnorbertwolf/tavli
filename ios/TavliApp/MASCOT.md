...
**Implemented** (`Views/TavTavMascot.swift`, wired into `GameView`, `OpeningRollView`,
`Tournament/TournamentStyle.swift`, and `Tournament/TournamentRootView.swift`; build verified
via `xcodebuild`). Two source images were provided and map to the two personas:
