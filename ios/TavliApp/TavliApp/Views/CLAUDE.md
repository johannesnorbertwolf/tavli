...
| `RootView.swift` | T10/#61 — app root: mode picker ↔ opening roll ↔ game; owns all save/load + stats wiring. #77: start-screen settings gear; a pinned preferred color collapses the per-game color pick; the starting-player setting can skip the opening roll; new/resumed sessions take their animation timings + AI-strength `searchConfig` (#108) from settings. **#134:** holds the `GameKitCoordinator` (a root `@StateObject`) and a "Play Online" entry; the online branch (`OnlineRootView`) is self-contained, bypassing the offline save/resume/stats wiring. **#145:** a "Single game / Best of 3" toggle on `ModePickerView` (default best-of-three) + a `@State match: MatchState?` runtime — the game-over hook advances the score, and the win overlay's "Next game"/"Rematch" build the next game (alternating starter, no opening-roll ceremony). In-memory only; a resumed autosave is a standalone single game. |
